...
export default class CalculusResponse {
  ...
  constructor(
    ...
    this.lifeCycle = new CalculusObjectResponseArray( lifeCycle.map(lc => {
      return CalculusObjectResponse.constructorLifePeriod(lc.getResult().toString(), lc);
    }),
    ...
    this.spiersAndRealization = new CalculusObjectResponseArray(
      spiersAndRealization.map((sr => {
        return CalculusObjectResponse.constructorLifePeriod(sr.getResult().toString(), sr);
      })),
      "Pináculos/Realizações de " + name);
    this.challenge =  new CalculusObjectResponseArray(
      challenge.map((c => {
        return CalculusObjectResponse.constructorLifePeriod(c.getResult().toString(), c);
      })),
      "Desafios de " + name);
    this.karmicNumber = new CalculusObjectResponseArray(karmicNumber.map(kc => {
      if(kc.IsKarmic){
        return CalculusObjectResponse.constructorString(kc.KarmicNumber.toString(), `${kc.KarmicNumber}: ${kc.Attribute}`);
      } else {
        return CalculusObjectResponse.constructorString("0", `Não há número karmico para ${kc.Attribute} de ${name}`);
      }
    }),
    ...
    this.quadrimesters= new CalculusObjectResponseArray(
      quadrimesters.map(q => {
        return CalculusObjectResponse.constructorQuadrimester(q.getResult().toString(), q, deep);
      }), 
